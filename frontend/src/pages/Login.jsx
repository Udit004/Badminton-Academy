...
const Login = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      console.error('Google login error:', error); // For debugging
      ...
      <div className="max-w-md w-full space-y-8 bg-black/40 backdrop-blur-sm p-8 rounded-xl border border-purple-500/20">
        ...
        <div>
          <button
            ...
            disabled={isLoading}
            className="w-full flex justify-center items-center py-2 px-4 border border-purple-500/20 rounded-md shadow-sm text-sm font-medium text-gray-300 bg-black/20 hover:bg-purple-500/20 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500 disabled:opacity-50"
          >
            <img
